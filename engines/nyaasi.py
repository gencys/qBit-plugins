...
class nyaasi(object):
    ...
    class NyaasiParser(HTMLParser):
        ...
        def start_a(self, attr):
            ...
            if (
                ...
                if not params["href"].startswith("magnet:?") and not params[
                    "href"
                ].endswith(".torrent"):
                    return
    ...
    def search(self, what, cat="all"):
        ...
        """
        filter = 0
        filter_txt = re.match(r'(trusted|noremakes)%3A', what)
        if filter_txt is not None:
            what = what[len(filter_txt.group(1)) + 3:]
            if filter_txt.group(1) == 'trusted':
                filter = 2
            if filter_txt.group(1) == 'noremakes':
                filter = 1
        url = str(
            "{0}/?f={1}&s=seeders&o=desc&c={2}&q={3}".format(
                self.url, filter, self.supported_categories.get(cat), what
            )
        )
